/**
 * Official BIP-340 known-answer test vectors, verbatim from
 * github.com/bitcoin/bips/blob/master/bip-0340/test-vectors.csv
 *
 * Indices 0–3 and 15–18 carry a secret key + aux and assert a specific
 * signature (signing KATs). Indices 4–14 are verification-only, deliberately
 * malformed cases that MUST be rejected (fail-closed KATs). Our hand-rolled
 * verify() must agree with the `expected` column on every row.
 */
export interface Bip340Vector {
  index: number;
  secretKey: string | null; // hex, or null for verify-only rows
  publicKey: string; // hex, 32-byte x-only
  auxRand: string | null; // hex, or null for verify-only rows
  message: string; // hex (may be empty)
  signature: string; // hex, 64-byte
  expected: boolean; // expected verification result
  comment: string;
}

export const BIP340_VECTORS: Bip340Vector[] = [
  {
    index: 0,
  ...
  },
  {
    ...
    expected: false,
    comment:
      'sG - eP is infinite. Test fails if has_even_y(inf) is defined as true and x(inf) as 0',
  },
  {
    index: 10,
    secretKey: null,
    ...
    expected: false,
    comment:
      'sG - eP is infinite. Test fails if has_even_y(inf) is defined as true and x(inf) as 1',
  },
  {
    index: 11,
    secretKey: null,
